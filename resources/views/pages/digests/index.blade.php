    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 sm:mb-0">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and organize your content digests</p>
            </div>
            ...
            <div class="max-w-2xl m-auto mt-16">
                <div class="text-center px-4">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mb-8 max-w-md mx-auto">
                        Create your first digest to start curating content for your audience. A digest is a collection of links and articles you can share.
                    </p>
                    <a href="{{ route('digests.create') }}" class="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white">
                        <svg class="fill-current shrink-0 mr-2" width="16" height="16" viewBox="0 0 16 16">
                            <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
        ...
        @else
            ...
            <div class="sm:flex sm:justify-between sm:items-center mb-6">
                <!-- Left: Search -->
                <div class="mb-4 sm:mb-0">
                    <form class="relative" method="GET" action="{{ route('digests.index') }}">
                        <label for="digest-search" class="sr-only">Search</label>
                        <input
                            id="digest-search"
                            name="search"
                ...
                <div class="flex items-center gap-2">
                    <a
                        ...
                        class="btn-sm px-3 py-1.5 rounded-lg text-sm font-medium transition {{ !request('status') ? 'bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-800' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 text-gray-600 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600' }}"
                    >
                        All
                    </a>
                    <a
                        href="{{ route('digests.index', ['status' => 'draft']) }}"
                        class="btn-sm px-3 py-1.5 rounded-lg text-sm font-medium transition {{ request('status') === 'draft' ? 'bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-800' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 text-gray-600 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600' }}"
                    >
                        Drafts
                    </a>
                    <a
                        href="{{ route('digests.index', ['status' => 'published']) }}"
                        class="btn-sm px-3 py-1.5 rounded-lg text-sm font-medium transition {{ request('status') === 'published' ? 'bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-800' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 text-gray-600 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600' }}"
                    >
                        Published
                    </a>
                </div>
            ...
            <div class="grid grid-cols-12 gap-4 mb-6">
                <div class="col-span-6 sm:col-span-3">
                    <div class="bg-white dark:bg-gray-800 shadow-xs rounded-lg p-4">
                        <div class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ $digests->total() }}</div>
                        <div class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Total Digests</div>
                    </div>
                </div>
                <div class="col-span-6 sm:col-span-3">
                    <div class="bg-white dark:bg-gray-800 shadow-xs rounded-lg p-4">
                        <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $digests->where('status', 'published')->count() }}</div>
                        <div class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Published</div>
                    </div>
                </div>
                <div class="col-span-6 sm:col-span-3">
                    <div class="bg-white dark:bg-gray-800 shadow-xs rounded-lg p-4">
                        <div class="text-2xl font-bold text-gray-600 dark:text-gray-400">{{ $digests->where('status', 'draft')->count() }}</div>
                        <div class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Drafts</div>
                    </div>
                </div>
                <div class="col-span-6 sm:col-span-3">
                    <div class="bg-white dark:bg-gray-800 shadow-xs rounded-lg p-4">
                        <div class="text-2xl font-bold text-violet-600 dark:text-violet-400">{{ $digests->sum(fn($d) => $d->items->count()) }}</div>
                        <div class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Total Items</div>
                    </div>
                </div>
            </div>
                ...
                @foreach($digests as $digest)
                    <div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-xs rounded-xl hover:shadow-md transition-shadow">
                        <div class="flex flex-col h-full">
                            ...
                            <div class="h-1 rounded-t-xl {{ $digest->status === 'published' ? 'bg-gradient-to-r from-green-400 to-green-500' : 'bg-gradient-to-r from-gray-300 to-gray-400 dark:from-gray-600 dark:to-gray-700' }}"></div>
                            <div class="flex flex-col h-full p-5">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="relative inline-flex" x-data="{ open: false }">
                                            ...
                                            <div class="origin-top-right z-10 absolute top-full right-0 min-w-36 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1" @click.outside="open = false" @keydown.escape.window="open = false" x-show="open" x-transition:enter="transition ease-out duration-200 transform" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-out duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-cloak>
                                                <ul>
                                                    <li>
                                                        <a class="font-medium text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 flex items-center gap-2 py-1.5 px-3" href="{{ route('digests.show', $digest) }}">
                                                            <svg class="w-4 h-4" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 3C3 3 1 8 1 8s2 5 7 5 7-5 7-5-2-5-7-5z"/><circle cx="8" cy="8" r="2"/></svg>
                                                            View
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="font-medium text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 flex items-center gap-2 py-1.5 px-3" href="{{ route('digests.edit', $digest) }}">
                                ...
                                <div class="grow mt-3">
                                    <a class="group" href="{{ route('digests.edit', $digest) }}">
                                        <h2 class="text-lg leading-snug font-semibold text-gray-800 dark:text-gray-100 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors mb-2">{{ $digest->title }}</h2>
                                    </a>
                                    <div class="flex items-center gap-4 text-sm">
                                        <div class="flex items-center gap-1.5 text-gray-500 dark:text-gray-400">
                                            ...
                                            <span>{{ $digest->items->count() }} {{ Str::plural('item', $digest->items->count()) }}</span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            @if($digest->published_at)
                                                <span class="font-medium text-green-600 dark:text-green-400">Published</span> {{ $digest->published_at->diffForHumans() }}
                                            @else
                                                Updated {{ $digest->updated_at->diffForHumans() }}
                                            @endif
                                        </div>
